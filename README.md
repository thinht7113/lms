# Hệ thống API Backend Quản lý Học tập Trực tuyến (LMS)

Tài liệu này cung cấp đặc tả chi tiết về thiết kế Cơ sở dữ liệu (PostgreSQL), kiến trúc Cổng kết nối API (FastAPI) và phân tích các quy tắc logic nghiệp vụ nâng cao được triển khai trong hệ thống LMS Backend.
...
Hệ thống API sử dụng cấu trúc định tuyến phân cấp thông qua `APIRouter` của FastAPI và được gộp lại tại [api_router](file:///d:/BT/LMS/lms-backend/app/api/v1/router.py) với tiền tố `/api/v1`.
...
| | `POST` | `/api/v1/quizzes/{quiz_id}/submit`| Student | Nộp bài làm trắc nghiệm và nhận điểm số tự động. |
...
*   *Vị trí file:* [cert_service.py:L15-L53](file:///d:/BT/LMS/lms-backend/app/services/cert_service.py#L15-L53)
...
*   *Vị trí file:* [order_service.py:L142-L172](file:///d:/BT/LMS/lms-backend/app/services/order_service.py#L142-L172)
...
*   *Vị trí file:* [quiz_service.py:L192-L205](file:///d:/BT/LMS/lms-backend/app/services/quiz_service.py#L192-L205)
...
*   *Vị trí file:* [quiz_schema:L13-L27](file:///d:/BT/LMS/lms-backend/app/schemas/quiz.py#L13-L27) và [quizzes.py:L70-L80](file:///d:/BT/LMS/lms-backend/app/api/v1/endpoints/quizzes.py#L70-L80)
...
*   *Vị trí file:* [cert_service.py:L55-L103](file:///d:/BT/LMS/lms-backend/app/services/cert_service.py#L55-L103)

---

## 4. Hướng dẫn cài đặt và Vận hành (Installation & Quickstart)
...
3.  **Thiết lập các biến môi trường (`.env`):**
    Tạo tệp `.env` tại thư mục gốc của dự án và cấu hình thông tin kết nối Database của bạn:
    ```env
    # Cấu hình Database kết nối bất đồng bộ (Asyncpg)
    DATABASE_URL=postgresql+asyncpg://postgres:matkhau@localhost:5432/lms_database
    
    # Cấu hình bảo mật JWT Token
    SECRET_KEY=CHUOI_SECRET_KEY_BAO_MAT_CUA_RIENG_BAN_123456
    ALGORITHM=HS256
    ACCESS_TOKEN_EXPIRE_MINUTES=1440
    ```

4.  **Chạy máy chủ phát triển (FastAPI):**
    ```bash
    # Chạy máy chủ phát triển cục bộ bằng Uvicorn với chế độ tự động tải lại khi đổi code
    $env:PYTHONPATH="."
    .venv\Scripts\python.exe app/main.py
    ```

5.  **Trải nghiệm API qua Swagger UI:**
    *   Mở trình duyệt và truy cập: **`http://localhost:8000/docs`**
    *   Hệ thống cơ sở dữ liệu sẽ tự động được khởi tạo các bảng tương ứng trên cơ sở dữ liệu PostgreSQL của bạn ngay trong lần khởi chạy đầu tiên.
