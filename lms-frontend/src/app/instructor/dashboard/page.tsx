...
export default function InstructorDashboardPage() {
  ...
  const dashboardStats = [
    { label: "Tổng học viên", value: (stats?.total_students || 0).toLocaleString(), icon: Users, color: "text-purple-600", bg: "bg-purple-600/10" },
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <h2 className="text-3xl font-black tracking-tight text-slate-900">Instructor Studio</h2>
            <p className="text-slate-500 font-medium mt-1">
            Chào mừng {user.ho_ten} quay trở lại. Hãy xem các chỉ số hoạt động của bạn hôm nay.
            </p>
        </div>
        <Link 
      ...
      {loading ? (
        <div className="flex h-72 items-center justify-center rounded-[2rem] border border-slate-200 bg-white shadow-sm">
          <RefreshCw className="h-9 w-9 animate-spin text-purple-600" />
        </div>
      ) : error ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2 bg-white border border-slate-200/60 rounded-[2rem] p-8 shadow-sm">
              <div className="flex items-center justify-between border-b border-slate-100 pb-4 mb-6">
                ...
                <Link href="/instructor/courses" className="text-xs font-black uppercase tracking-widest text-purple-600 hover:text-purple-800">
                  Xem tất cả
                </Link>
              </div>
              ...
              {courses.length === 0 ? (
                <div className="flex min-h-[260px] flex-col items-center justify-center rounded-2xl border-2 border-dashed border-slate-200 bg-slate-50/50 text-center">
                  ...
                  <Link href="/instructor/courses/create" className="mt-4 rounded-2xl bg-purple-600 px-5 py-3 text-sm font-black text-white hover:bg-purple-700">
                    Bắt đầu ngay
                  </Link>
                </div>
              ) : (
                  ...
                  {courses.slice(0, 6).map((course) => (
                    <div key={course.id} className="flex flex-col gap-3 rounded-2xl border border-slate-100 bg-slate-50/60 p-4 sm:flex-row sm:items-center sm:justify-between hover:bg-slate-50 transition-colors">
                      <div className="flex items-center gap-4">
                        <div className="w-12 h-12 rounded-xl bg-purple-100 border border-purple-200 overflow-hidden shrink-0">
                            {course.anh_dai_dien ? (
                                <img src={course.anh_dai_dien} className="w-full h-full object-cover" alt="" />
                            ) : (
                                <div className="w-full h-full flex items-center justify-center text-purple-600 font-black text-xs">COURSE</div>
                            )}
                        </div>
                        <div>
            ...
            <div className="lg:col-span-1 space-y-6">
                <div className="bg-slate-900 rounded-[2rem] p-8 text-white shadow-xl relative overflow-hidden group">
                    <div className="absolute top-0 right-0 w-32 h-32 bg-purple-600/30 rounded-full blur-3xl -mr-10 -mt-10 group-hover:bg-purple-600/50 transition-colors"></div>
                    <div className="relative z-10 space-y-6">
                        <div className="space-y-2">
                            <p className="text-[10px] font-black uppercase tracking-widest text-purple-400">Doanh thu tạm tính</p>
                            <p className="text-3xl font-black">{(stats?.total_revenue * 0.7 || 0).toLocaleString('vi-VN')} đ</p>
                        </div>
                        <p className="text-xs text-slate-400 font-medium leading-relaxed">Số dư này có thể rút sau khi học viên thanh toán thành công và hết hạn hoàn tiền.</p>
                        <button onClick={() => router.push('/instructor/revenue')} className="w-full bg-white text-slate-900 py-3 rounded-xl text-xs font-black uppercase tracking-widest hover:bg-purple-100 transition-colors">
                            Xem chi tiết ví
                        </button>
                    </div>
                </div>
