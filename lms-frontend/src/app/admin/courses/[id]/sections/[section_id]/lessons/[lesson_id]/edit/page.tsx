    ...
    useEffect(() => {
        const fetchLessonData = async () => {
            try {
                ...
                const lessonRes = await fetch(`${API_BASE_URL}/dynamic-admin/lessons/${lessonId}`, {
                ...
                const contentsRes = await fetch(`${API_BASE_URL}/dynamic-admin/lesson-contents?filter_col=ma_bai_hoc&filter_val=${lessonId}&limit=100`, {
                    headers: { "Authorization": `Bearer ${token}` }
                });
                if (contentsRes.ok) {
        ...
        try {
            ...
            const lessonRes = await fetch(`${API_BASE_URL}/dynamic-admin/lessons/${lessonId}`, {
                ...
                body: JSON.stringify({
                    ...
                    tieu_de: tieuDe,
                    thoi_luong: (typeof thoiLuong === "number" ? thoiLuong : 0) * 60, // Đổi phút ra giây
                    thu_tu: thuTu,
                    xem_truoc: xemTruoc,
            ...
            for (const delId of deletedBlockIds) {
                await fetch(`${API_BASE_URL}/dynamic-admin/lesson-contents/${delId}`, {
                    method: "DELETE",
                    headers
                });
            }
            ...
            for (let i = 0; i < blocks.length; i++) {
                ...
                if (block.isExisting) {
                    // Update - PUT
                    await fetch(`${API_BASE_URL}/dynamic-admin/lesson-contents/${block.id}`, {
                        method: "PUT",
                        headers,
                        body: JSON.stringify({
        ...
        <div className="h-full w-full space-y-6 pb-20 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="bg-card border border-border/60 rounded-[2rem] p-6 sm:p-8 shadow-sm space-y-6">
                <div className="flex items-center justify-between border-b border-border/40 pb-4">
                    <div>
                        <h2 className="text-lg font-black tracking-tight">Multimedia Builder</h2>
